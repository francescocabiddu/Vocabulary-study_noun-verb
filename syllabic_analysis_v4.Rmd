...
toc-title: 'Vocabulary paper: results - syllabic analysis (v4)'
---
...
Phoneme models analysis on mother's vocabulary showed longer sequences in learned bisyllabic and trisyllabic words (and shorter sequences in not learned trisyllabic words, 1.4.). When learned words matched not learned words on frequency, the differences disappeared (1.5.).
Phoneme models analysis on child's vocabulary showed that the differences are still found when learned words matched not learned words on frequency (2.2.). In 2.2. bisyllabic words showed a significant difference for 4 and 5-phoneme sequences. Trisyllabic words showed a tendency to significance for 4 and 5-phoneme sequences because of the large variation of average number of models across children (due to some small trisyllabic matched learned words sample sizes, see table "Summary number of words" in section 1.5.).

<br>

# 1. Counts on Mothers' utterances

### 1.1. Monosyllabic types in bisyllabic mothers' words
...
### 1.2. Monosyllabic types in bisyllabic mothers' words over time
...
### 1.3. Monosyllabic tokens in bisyllabic mothers' words over time
...
Learned and not learned words in every set statistically differ in frequency. In the following, we compare monosyllabic, bisyllabic and trisyllabic learned and not learned words. See 1.5 for the same analysis using a subset of learned words that matches not learned words on frequency. 
...
**Figure .** Mean and standard errors of number of 2-phoneme 'models' in learned and not-learned trisyllabic nouns
...
### 1.5. phoneme models over time matching on frequency
Repeating (1.4.) matching learned and not learned words on frequency 
...
tests <- list(t.test(summary_seq_tokens_tri_learned_match$mean_seq2[1:12],
                 summary_seq_tokens_tri_not_learned$mean_seq2[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_tri_learned_match$mean_seq3[1:12],
                 summary_seq_tokens_tri_not_learned$mean_seq3[1:12],
                 paired = TRUE),
...
**Figure .** Mean and standard errors of number of 2-phoneme 'models' in learned and not-learned matched trisyllabic nouns

<br>

Apart from a difference in 2-phoneme sequences between learned and not learned trisyllabic nouns, no differences were found matching learned and not learned words on frequency. In 2. the phoneme sequences analysis (1.4 and 1.5) is repeated, counting models occurrencies in the child's vocabulary rather than the mother's.

<br>

# 2. Counts on Children's utterances
Models in children's vocabulary rather than the mothers'

<br>

### 2.1. phoneme models over time
Average number of 2-phoneme, 3-phoneme, 4-phoneme and 5-phoneme 'models' (tokens considered) there are in child's vocabulary up until current stage.
...
```{r echo=FALSE}
summary_seq_tokens_mono_learned_chi %>% round_df(2) %>% kable()
```
...
```{r echo=FALSE}
summary_seq_tokens_mono_not_learned_chi %>% round_df(2) %>% kable()
```
...
```{r echo=FALSE}
tests <- list(t.test(summary_seq_tokens_mono_learned_chi$mean_seq2[1:12],
                 summary_seq_tokens_mono_not_learned_chi$mean_seq2[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_mono_learned_chi$mean_seq3[1:12],
                 summary_seq_tokens_mono_not_learned_chi$mean_seq3[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_mono_learned_chi$mean_seq4[1:12],
                 summary_seq_tokens_mono_not_learned_chi$mean_seq4[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_mono_learned_chi$mean_seq5[1:12],
                 summary_seq_tokens_mono_not_learned_chi$mean_seq5[1:12],
                 paired = TRUE))
...
```{r echo=FALSE}
summary_seq_tokens_learned_chi %>% round_df(2) %>% kable()
```
...
```{r echo=FALSE}
summary_seq_tokens_not_learned_chi %>% round_df(2) %>% kable()
```
...
```{r echo=FALSE}
tests <- list(t.test(summary_seq_tokens_learned_chi$mean_seq2[1:12],
                 summary_seq_tokens_not_learned_chi$mean_seq2[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_learned_chi$mean_seq3[1:12],
                 summary_seq_tokens_not_learned_chi$mean_seq3[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_learned_chi$mean_seq4[1:12],
                 summary_seq_tokens_not_learned_chi$mean_seq4[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_learned_chi$mean_seq5[1:12],
                 summary_seq_tokens_not_learned_chi$mean_seq5[1:12],
                 paired = TRUE))
...
summary_seq_tokens_learned_chi %>%
  long_summary(" Learned") %>%
  rbind(summary_seq_tokens_not_learned_chi %>%
          long_summary(" Not learned")) %>%
  filter(Sequence %in% c("3")) %>%
  ggplot(aes(x=Sequence, y=Mean, fill=Set)) +
  stat_summary(fun.y="mean", geom="bar", position=position_dodge(width=0.95))+
  stat_summary(geom = "errorbar", 
               fun.data = mean_se, position = position_dodge(width=0.95),
...
**Figure .** Mean and standard errors of number of 3-phoneme 'models' in learned and not-learned bisyllabic nouns
...
```{r echo=FALSE}
summary_seq_tokens_tri_learned_chi %>% round_df(2) %>% kable()
```
...
```{r echo=FALSE}
summary_seq_tokens_tri_not_learned_chi %>% round_df(2) %>% kable()
```
...
```{r echo=FALSE}
tests <- list(t.test(summary_seq_tokens_tri_learned_chi$mean_seq2[1:12],
                 summary_seq_tokens_tri_not_learned_chi$mean_seq2[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_tri_learned_chi$mean_seq3[1:12],
                 summary_seq_tokens_tri_not_learned_chi$mean_seq3[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_tri_learned_chi$mean_seq4[1:12],
                 summary_seq_tokens_tri_not_learned_chi$mean_seq4[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_tri_learned_chi$mean_seq5[1:12],
                 summary_seq_tokens_tri_not_learned_chi$mean_seq5[1:12],
                 paired = TRUE))
...
summary_seq_tokens_tri_learned_chi %>%
  long_summary(" Learned") %>%
  rbind(summary_seq_tokens_tri_not_learned_chi %>%
          long_summary(" Not learned")) %>%
  filter(Sequence %in% c("4", "5")) %>%
  ggplot(aes(x=Sequence, y=Mean, fill=Set)) +
  stat_summary(fun.y="mean", geom="bar", position=position_dodge(width=0.95))+
...
Counting phoneme models in children's vocabulary showed a similar pattern to mothers's vocabulary (1.4.). In 2.2. learned and not learned words matched on frequency are compared counting models in children's vocabulary. 

<br>

### 2.2. phoneme models over time matching on frequency
...
```{r echo=FALSE}
summary_seq_tokens_mono_learned_match_chi %>% round_df(2) %>% kable()
```
...
```{r echo=FALSE}
tests <- list(t.test(summary_seq_tokens_mono_learned_match_chi$mean_seq2[1:12],
                 summary_seq_tokens_mono_not_learned_chi$mean_seq2[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_mono_learned_match_chi$mean_seq3[1:12],
                 summary_seq_tokens_mono_not_learned_chi$mean_seq3[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_mono_learned_match_chi$mean_seq4[1:12],
                 summary_seq_tokens_mono_not_learned_chi$mean_seq4[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_mono_learned_match_chi$mean_seq5[1:12],
                 summary_seq_tokens_mono_not_learned_chi$mean_seq5[1:12],
                 paired = TRUE))
...
```{r echo=FALSE}
summary_seq_tokens_bis_learned_match_chi %>% round_df(2) %>% kable()
```
...
```{r echo=FALSE}
tests <- list(t.test(summary_seq_tokens_bis_learned_match_chi$mean_seq2[1:12] %>% round(2),
                 summary_seq_tokens_not_learned_chi$mean_seq2[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_bis_learned_match_chi$mean_seq3[1:12],
                 summary_seq_tokens_not_learned_chi$mean_seq3[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_bis_learned_match_chi$mean_seq4[1:12],
                 summary_seq_tokens_not_learned_chi$mean_seq4[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_bis_learned_match_chi$mean_seq5[1:12],
                 summary_seq_tokens_not_learned_chi$mean_seq5[1:12],
                 paired = TRUE))
...
**When comparing matched learned and not learned bisyllabic words, a statistical difference is found for 4 and 5-phoneme models in children's vocabulary.**
...
```{r echo=FALSE}
summary_seq_tokens_tri_learned_match_chi %>% round_df(2) %>% kable()
```
...
```{r echo=FALSE}
tests <- list(t.test(summary_seq_tokens_tri_learned_match_chi$mean_seq2[1:12],
                 summary_seq_tokens_tri_not_learned_chi$mean_seq2[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_tri_learned_match_chi$mean_seq3[1:12],
                 summary_seq_tokens_tri_not_learned_chi$mean_seq3[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_tri_learned_match_chi$mean_seq4[1:12],
                 summary_seq_tokens_tri_not_learned_chi$mean_seq4[1:12],
                 paired = TRUE),
          t.test(summary_seq_tokens_tri_learned_match_chi$mean_seq5[1:12],
                 summary_seq_tokens_tri_not_learned_chi$mean_seq5[1:12],
                 paired = TRUE))
...
**Figure .** Mean and standard errors of number of 4-phoneme and 5-phoneme 'models' in learned and not-learned matched trisyllabic nouns

<br>


**When comparing matched learned and not learned trisyllabic words, tendencies to statistical difference are present for 4 and 5-phoneme models in children's vocabulary, showing more models in children's vocabulary for learned words.**
**A reason why these comparisons didn't reach statistical significance could be found in the small sample size of learned trisyllabic models (in child's vocabulary) that increased the variation of models average number**
